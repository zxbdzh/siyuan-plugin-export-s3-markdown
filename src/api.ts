...
export async function testS3Connection(
  ...
  const client = initS3Client(endpoint, accessKey, secretKey, region);
  ...
  try {
    let response = await client.send(putObjectCommand);
    if (response.$metadata.httpStatusCode === 200) await pushMsg("测试成功");
    else {
      await pushErrMsg("测试失败");
      return;
    }
    // 删除测试文件
    await client.send(deleteObjectCommand);
  } catch (e) {
    await pushErrMsg("测试失败, 错误：" + e);
  }
}
...
export async function uploadToPicList(
  ...
  if (file instanceof File) {
    ...
    // 将Buffer转换为ArrayBufferView，然后再转换为File对象
    const uint8Array = new Uint8Array(file);
    uploadFile = new File([uint8Array], "image.jpg", { type: "image/jpeg" });
  } else {
    throw new Error("不支持的文件类型，必须是File或Buffer对象");
  }

  formData.append(fieldname, uploadFile);

  const url = apiKey
    ? `${serverUrl}/upload?key=${apiKey}`
    : `${serverUrl}/upload`;
  ...
  try {
    ...
    if (
      ...
      return response.data.result[0]; // 返回上传后的URL
    } else {
      throw new Error("上传失败: " + (response.data.message || "未知错误"));
    }
  } catch (error) {
    console.error("PicList上传错误:", error);
    throw error;
  }
}

// 测试PicList连接
export async function testPicListConnection(
  serverUrl: string,
  apiKey?: string,
) {
  try {
    ...
    if (response.data.success) {
      await pushMsg("PicList连接测试成功");
      return true;
    } else {
      await pushErrMsg(
        "PicList连接测试失败: " + (response.data.message || "未知错误"),
      );
      return false;
    }
  } catch (error) {
